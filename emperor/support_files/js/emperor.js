...
function displayTaxaLabels(taxaLevel){
  if(!jQuery.isEmptyObject(g_taxaPositions)){
    ...
    for( var i = 0; i < g_taxaPositions.length; i++){
      ...
      $("#taxalabels").append(
        '<label id="' + i + '_taxalabel" class="unselectable labels" '+
...
function drawTaxa(){
  ...
  for(var i = 0; i < g_taxaPositions.length; i++){
    ...
    var taxaVector = makeLine(
        [0, 0, 0,],
        [taxaPos['x'], taxaPos['y'], taxaPos['z']],
        whiteColor, 2);
...
$(document).ready(function() {
  ...
  function init() {
    ...
    for(var i = 0 ; i < g_plotIds.length; i++){
      ...
      $("#labels").append(
        '<label id="'+divid+'_label" class="unselectable labels"' +
          'style="position:absolute; left:' + parseInt(coords['x']) + 'px; top:' + parseInt(coords['y'])+'px;">' +
            sid +
        '</label>');
    }
...
function chosen_highlight_callback(chosen, highlight){
  // ignore if not the colorbycombo
  // ignore if it doesn't have context, not the element that we are looking for
  if (chosen.form_field.id != 'colorbycombo' || !('context' in highlight))
    return
